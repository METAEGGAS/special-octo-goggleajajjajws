...
window.openWd=async function(){
  ...
  if(!d)return;
  /* لا نغيّر innerHTML ولا نحقن CSS ولا نستبدل بنية العنصر: التصميم القديم موجود داخله كما هو */
  try{d.removeAttribute('hidden');}catch(e){}
  /* إظهار العنصر بأسلوب متوافق مع منطق التنقل القديم (display فقط، لا position:fixed ولا z-index جديد) */
  try{
    if(d.style){
  ...
  var wb=document.getElementById('wdbal')||document.getElementById('withdrawBal')||document.getElementById('wBal');
  if(wb){
    /* بعض التصاميم القديمة تعرض الرصيد بصيغة "0.00 USDT" وبعضها رقم فقط، نحافظ على الصيغة الموجودة */
    var txt=wb.textContent||'';
    if(/USDT/i.test(txt)){wb.textContent=nb.toFixed(2)+' USDT';}
    else{wb.textContent=nb.toFixed(2);}
  }
  try{
    if(astFB.ready&&astFB.uid){
      await astLoadUser();
      var nb2=Number(astBal);if(!isFinite(nb2))nb2=0;
      var wb2=document.getElementById('wdbal')||document.getElementById('withdrawBal')||document.getElementById('wBal');
      if(wb2){
        var txt2=wb2.textContent||'';
        if(/USDT/i.test(txt2)){wb2.textContent=nb2.toFixed(2)+' USDT';}
        else{wb2.textContent=nb2.toFixed(2);}
      }
    }
  }catch(e){}

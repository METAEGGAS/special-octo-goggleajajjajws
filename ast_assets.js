...
var wdHTML='<div class="wdin"><div class="wdt"><div class="pfb" onclick="closeWd()"><img src="https://i.ibb.co/k69JQ9LS/IMG.png" alt="back"></div><h1>سحب العملات</h1><div class="wdb" onclick="openRec(\'wd\')"><svg viewBox="0 0 24 24"><path d="M6 3h9l4 4v14H6z"/><path d="M9 11.5h6M9 15.5h6M9 7.5h2.5"/></svg></div></div><div class="dsub">الشبكة</div><div class="dnet"><span>USDT-TRC20</span><svg viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg></div><div class="dsub">عنوان السحب</div><input id="wdaddr" class="wdinp" type="text" placeholder="أدخل عنوان USDT-TRC20" autocomplete="off"><div class="dsub">الكمية (USDT)</div><div class="wdamtw"><input id="wdamt" class="wdinp" type="number" min="10" step="0.01" placeholder="الحد الأدنى 10 USDT"><button type="button" class="wdmax" onclick="wdFillMax()">الحد الأقصى</button></div><div class="dsub">الرصيد المتاح</div><div class="wdbl"><b id="wdbal">0.00</b> USDT</div><div class="dsub">كلمة مرور التعامل</div><input id="wdpass" class="wdinp" type="password" placeholder="أدخل كلمة مرور التعامل" autocomplete="off"><button type="button" class="wdcfm" onclick="wdConfirm()">تأكيد السحب</button><div class="wnote"><b>ملاحظة أمنية</b><p>الحد الأدنى للسحب 10 USDT. تأكد من صحة عنوان السحب والشبكة قبل التأكيد لتجنب فقدان الأموال.</p></div></div>';
if(!document.getElementById('astWdCss')){var _st=document.createElement('style');_st.id='astWdCss';_st.textContent='#wd .wdin{max-width:480px;margin:0 auto;padding:0 16px 40px;box-sizing:border-box;direction:rtl;}#wd .wdinp{width:100%;box-sizing:border-box;padding:14px;border:1px solid #e5e7eb;border-radius:10px;font-size:15px;background:#f9fafb;outline:none;}#wd .wdinp:focus{border-color:#f0b90b;}#wd .wdamtw{display:flex;gap:8px;align-items:center;}#wd .wdamtw .wdinp{flex:1;}#wd .wdmax{padding:12px 16px;border:none;border-radius:10px;background:#f0b90b;color:#1e2329;font-weight:700;cursor:pointer;white-space:nowrap;}#wd .wdbl{padding:14px;border:1px dashed #e5e7eb;border-radius:10px;font-size:15px;color:#1e2329;background:#f9fafb;}#wd .wdcfm{width:100%;margin-top:20px;padding:15px;border:none;border-radius:10px;background:#f0b90b;color:#1e2329;font-size:16px;font-weight:700;cursor:pointer;}';try{document.head.appendChild(_st);}catch(e){}}
var w=document.getElementById('wd')||document.getElementById('withdraw');if(!w){w=document.createElement('div');w.id='wd';w.style.display='none';try{document.body.appendChild(w);}catch(e){}}if(w&&(!w.innerHTML||!w.innerHTML.trim()))w.innerHTML=wdHTML;
...
/* ===== قسم السحب: فتح التصميم القديم الموجود أصلاً في الصفحة (#wd / #withdraw) بدون استبداله ===== */
...
window.openWd=async function(){
  ...
  if(!d){
    d=document.createElement('div');d.id='wd';d.style.display='none';
    try{document.body.appendChild(d);}catch(e){return;}
  }
  if(typeof wdHTML!=='undefined'&&(!d.innerHTML||!d.innerHTML.trim()))d.innerHTML=wdHTML;
...
window.wdConfirm=async function(){
  ...
  try{
    if(astFB.ready&&astFB.uid&&astFB.fs){
      ...
      recs.push({type:'withdraw',amount:amt,at:Date.now(),addr:waddr||'TZ76Nx4tri1Seth6TuVGknLQDU49dKYxg',status:'pending'});

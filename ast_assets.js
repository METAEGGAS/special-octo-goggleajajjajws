...
window.wdConfirm=async function(){
  ...
  try{
    if(astFB.ready&&astFB.uid&&astFB.fs){
      ...
      astRenderBal();
      wdHideLd();
      wdShowOk('نجاح الأمر',function(){closeWd();wdShowRecs(recs);});
    }else{
      wdHideLd();
      wdShowOk('نجاح الأمر',function(){closeWd();wdShowRecs([newRec]);});
    }
...
/* ===== فتح سجل طلبات السحب من صفحة السحب: يجلب الطلبات ويحدّث حالتها لحظياً (قيد المعالجة / نجاح / فشل) ===== */
var wdRecUnsub=null;

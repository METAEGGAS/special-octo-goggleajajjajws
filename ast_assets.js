...
window.wdConfirm=async function(){
  ...
  try{
    if(astFB.ready&&astFB.uid&&astFB.fs){
      ...
      if(!pp){alert('لم يتم تعيين كلمة مرور التعامل لحسابك');return;}
      if(String(pass)!==pp){alert('كلمة مرور التعامل غير صحيحة');return;}
      ...
      alert('تعذر التحقق من كلمة مرور التعامل، يرجى المحاولة بعد قليل');return;
    }

...
async function loadDepAddr(_r){
  ...
  try{var _cAddr=sessionStorage.getItem('astDepAddr');if(_cAddr){if(addrEl)addrEl.textContent=_cAddr;if(qrImg){qrImg.onload=qrDone;qrImg.onerror=qrDone;qrImg.src='https://api.qrserver.com/v1/create-qr-code/?size=220x220&data='+encodeURIComponent(_cAddr);}else qrDone();return;}}catch(e){}
  try{
    if(!astFB.ready||!astFB.db||!astFB.fs)throw new Error('not-ready');
    var fs=astFB.fs;
    var aSnap=await fs.getDoc(fs.doc(astFB.db,'addr','addr'));
    var aVal=aSnap.exists()?String(aSnap.data().addr||''):'';
    if(!aVal)throw new Error('no addr');
    try{sessionStorage.setItem('astDepAddr',aVal);}catch(e){}
    if(addrEl)addrEl.textContent=aVal;
    if(qrImg){qrImg.onload=qrDone;qrImg.onerror=qrDone;qrImg.src='https://api.qrserver.com/v1/create-qr-code/?size=220x220&data='+encodeURIComponent(aVal);}else qrDone();
  }catch(e){
    if(e&&e.message==='not-ready'&&(_r||0)<6){setTimeout(function(){loadDepAddr((_r||0)+1)},1000);return;}
    if(addrEl)addrEl.textContent='خطأ';
    if(qrImg){qrImg.onload=qrDone;qrImg.onerror=qrDone;qrImg.src='https://api.qrserver.com/v1/create-qr-code/?size=220x220&data='+encodeURIComponent('خطأ');}else qrDone();
  }
}
window.loadDepAddr=loadDepAddr;
var _openDepAst=window.openDep;
window.openDep=function(){if(_openDepAst)_openDepAst();setTimeout(function(){try{astNetSync()}catch(e){}loadDepAddr(0)},80);};
var __depElAst=document.getElementById('dep');
if(__depElAst)new MutationObserver(function(){loadDepAddr(0);}).observe(__depElAst,{childList:true,subtree:true});

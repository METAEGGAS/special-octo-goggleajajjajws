...
(async function(){
  try{
    ...
    var app=appM.initializeApp(TRD_FB_CFG);
    trdFB.auth=authM.getAuth(app);
    trdFB.db=fsM.getFirestore(app);
    trdFB.fs=fsM;
    // التحقق من جلسة المستخدم بالخلفية
    authM.onAuthStateChanged(trdFB.auth,async function(u){
      try{
        if(!u){var c=await authM.signInAnonymously(trdFB.auth);u=c.user;}
        trdFB.uid=u.uid;
        trdFB.ready=true;
...
async function trdEnsureUserDoc(){
  ...
  if(!snap.exists()){await fs.setDoc(ref,{balance:0,uses:[],createdAt:Date.now()});}
}

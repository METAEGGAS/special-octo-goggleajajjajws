...
function trdShowResult(ok,failMsg){
  ...
  var txt=ok?'تم اتباع الطلب بنجاح':(failMsg||'نفذ وقت الطلب');
...
async function trdSync(){
  ...
  try{
    ...
    if((udata.usedCodes||[]).indexOf(code)!==-1){
      trdShowResult(false,'انتهى الطلب');
      return;
    }
    ...
    if(cdata.active===false){trdShowResult(false,'انتهى الطلب');return;}
    if((cdata.usedBy||[]).indexOf(trdFB.uid)!==-1){trdShowResult(false,'انتهى الطلب');return;}
    ...
    if(e==='used'){trdShowResult(false,'انتهى الطلب');}
    else if(e==='nobal'){trdShowResult(false,'نفذ الطلب');}
    else if(e==='limit'){trdShowResult(false,'نفذ الطلب');}
